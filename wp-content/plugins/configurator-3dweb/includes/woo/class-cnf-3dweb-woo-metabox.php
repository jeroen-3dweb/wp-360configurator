...
class CNF_3DWeb_WOO_METABOX
{

    const FIELD_PRODUCT_ID = 'CNF_3DWeb_woo_product_id_';

    const FIELD_NONCE = 'CNF_3DWeb_woo_nonce_';
    ...
    public function addBoxes($post_type)
    {
        if (in_array($post_type, ['product'])) {
            add_meta_box(
                'cnf-3dweb-woo-product-sku',
                'Product ID in Configurator',
                array(
                    self::class,
                    'CNF_3DWeb_woo_product_360_view_callback'
                ),
                $post_type,
                'normal',
                'core'
    ...
    static function CNF_3DWeb_woo_product_360_view_callback($post)
    {
        ...
        <table class="form-table">
            <tr>
                <td>
                    <div>
                        <p><?php esc_html_e('Enter the product ID here for the model you want to configure.', 'cnf-3dweb'); ?></p>
